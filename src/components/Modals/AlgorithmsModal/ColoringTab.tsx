...
import { runHlpEdgeColoringAnimation, runMisraGriesAnimation } from '@/services/algorithms/coloring';
...
export const ColoringTab = forwardRef<ColoringTabRef, ColoringTabProps>(({ isOpen: _ }, ref) => {
    ...
    const handleRun = () => {
        ...
        try {
            const animation = families.includes('hlp')
                ? runHlpEdgeColoringAnimation(activeGraph, { algorithm: 'hlp-edge-coloring' })
                : runMisraGriesAnimation(activeGraph, { algorithm: 'misra-gries' });
