...
describe('animationStore', () => {
    ...
    it('initAnimation creates tab with idle status and animation set', () => {
        ...
        const tabs = useAnimationStore.getState().tabs;
        expect(tabs[TAB]?.status).toBe('idle');
        expect(tabs[TAB]?.animation).toEqual(mockAnimation);
        expect(tabs[TAB]?.currentStepIndex).toBe(0);
    });
    ...
    it('stop resets tab to idle with no animation', () => {
        ...
        const tabs = useAnimationStore.getState().tabs;
        expect(tabs[TAB]?.status).toBe('idle');
        expect(tabs[TAB]?.animation).toBeNull();
        expect(tabs[TAB]?.currentStepIndex).toBe(0);
    });
    ...
    it('stepForward at last step sets status to finished', () => {
        ...
        const tabs = useAnimationStore.getState().tabs;
        expect(tabs[TAB]?.status).toBe('finished');
        expect(tabs[TAB]?.currentStepIndex).toBe(2);
    });
