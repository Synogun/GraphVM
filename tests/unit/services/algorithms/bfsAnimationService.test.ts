...
describe('runBFSAnimation', () => {
    ...
    it('returns algorithm "bfs"', () => {
        const result = runBFSAnimation({ graph: core, startNodeId: 'a', directed: false, onlySelected: false });
    ...
    it('throws when start node does not exist', () => {
        expect(() =>
            runBFSAnimation({ graph: core, startNodeId: 'nonexistent', directed: false, onlySelected: false })
        ).toThrow();
    });
    ...
    it('directed BFS only follows outgoing edges', () => {
        ...
        const result = runBFSAnimation({ graph: core, startNodeId: 'c', directed: true, onlySelected: false });
        const visitedIds = result.steps.filter(s => s.operation === 'visit').map(s => s.nodeId);
        expect(visitedIds).toContain('c');
    ...
    it('onlySelected only traverses selected nodes', () => {
        ...
        core.$id('a').select();
        core.$id('b').select();
        const result = runBFSAnimation({ graph: core, startNodeId: 'a', directed: false, onlySelected: true });
        const visitedIds = result.steps.filter(s => s.operation === 'visit').map(s => s.nodeId);
        expect(visitedIds).toContain('a');
        expect(visitedIds).toContain('b');
    ...
    it('throws when onlySelected is true and start node is not selected', () => {
        ...
        expect(() =>
            runBFSAnimation({ graph: core, startNodeId: 'a', directed: false, onlySelected: true })
        ).toThrow();
    });
});
