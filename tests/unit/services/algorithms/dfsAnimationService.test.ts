...
describe('runDFSAnimation', () => {
    ...
    it('returns algorithm "dfs"', () => {
        const result = runDFSAnimation({ graph: core, startNodeId: 'a', directed: false, onlySelected: false });
    ...
    it('visits all nodes in a linear graph', () => {
        const result = runDFSAnimation({ graph: core, startNodeId: 'a', directed: false, onlySelected: false });
        const visitedIds = result.steps.filter(s => s.operation === 'visit').map(s => s.nodeId);
    ...
    it('throws when start node does not exist', () => {
        expect(() =>
            runDFSAnimation({ graph: core, startNodeId: 'nonexistent', directed: false, onlySelected: false })
        ).toThrow();
    });
    ...
    it('directed DFS only follows outgoing edges', () => {
        const result = runDFSAnimation({ graph: core, startNodeId: 'c', directed: true, onlySelected: false });
        const visitedIds = result.steps.filter(s => s.operation === 'visit').map(s => s.nodeId);
        expect(visitedIds).toContain('c');
    ...
    it('onlySelected only traverses selected nodes', () => {
        core.$id('a').select();
        core.$id('b').select();
        const result = runDFSAnimation({ graph: core, startNodeId: 'a', directed: false, onlySelected: true });
        const visitedIds = result.steps.filter(s => s.operation === 'visit').map(s => s.nodeId);
        expect(visitedIds).toContain('a');
        expect(visitedIds).toContain('b');
    ...
    it('throws when onlySelected is true and start node is not selected', () => {
        expect(() =>
            runDFSAnimation({ graph: core, startNodeId: 'a', directed: false, onlySelected: true })
        ).toThrow();
    });
});
